...
public class MainServlet extends HttpServlet {
	...
	private void doLogIn(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String id = request.getParameter("_________");
		String password = request.getParameter("_________");
		String name = "김싸피";
		...
		______ userId = new _______("userId" , id);
		// 쿠키 소멸 시간 설정 ( 1일 )
		userId.__________(60*60*24);
		// 응답 헤더에 쿠키 추가
		response.________(userId);
		
		String msg = "로그인 성공!";
		
		// session 가져오기
		____________ session = ___________;
		
		// id, password 비교하여 동작 구분
		if(id.equals("ssafy") && password.equals("1234")) {
			// id, password가 일치하면 session에 사용자 정보 저장
			session.____________("loginUser", ________);
		} else {
	...
	private void doLogOut(HttpServletRequest request, HttpServletResponse response) throws IOException {
		...
		// session 가져오기
		__________ session = _______________;
		
		// session 만료시키기
		session.____________;
		
		// 리다이렉트 시키기
		response.____________("./regist.jsp");
	}
}
